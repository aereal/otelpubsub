...
type producerFunc func(e *sub.Entity) iter.Seq[attribute.KeyValue]

func (f producerFunc) ProduceSNSProcessSpanAttributes(e *sub.Entity) iter.Seq[attribute.KeyValue] {
	return f(e)
}
...
func TestStartProcessSpan_WithAttributeProducers(t *testing.T) {
	...
	tp := sdktrace.NewTracerProvider(sdktrace.WithSyncer(exporter))
	entity := &sub.Entity{}
	producer := producerFunc(func(_ *sub.Entity) iter.Seq[attribute.KeyValue] {
		return func(yield func(attribute.KeyValue) bool) {
			yield(attribute.String("test.key", "test.value"))
		}
	})

	_, span := sub.StartProcessSpan(t.Context(), entity,
